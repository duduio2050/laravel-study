...
final class RepositoryBuilder
{
     ...
     *
     * Accepts either a reader instance, or a classes-string for an adapter. If
    ...
    public function addReader($reader)
    {
        if (!(\is_string($reader) && self::isAnAdapterClass($reader)) && !($reader instanceof ReaderInterface)) {
            throw new InvalidArgumentException(
                \sprintf(
                    'Expected either an instance of %s or a classes-string implementing %s',
     ...
     *
     * Accepts either a writer instance, or a classes-string for an adapter. If
     * the adapter is not supported, then we silently skip adding it.
     *
     ...
     *
     * Accepts either an adapter instance, or a classes-string for an adapter. If
